        ...
        <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fithighlight-current-row style="width: 100%" height="500">
            ...
            <el-table-column min-width="300px" align="center" label="问题" prop="title">
                <template scope="scope">
                    ...
                    <span style="color:#337ab7;"><router-link :to="{ path: '/homefm/form/' + scope.row.id }">{{scope.row.title}}</router-link></span>
                </template>
            </el-table-column>
...
<script type="text/ECMAScript-6">
    ...
    import { uphomefm } from 'api/homefm';
    import { sorthomefm } from 'api/homefm';
    import { delhomefm } from 'api/homefm';
        ...
        methods: {
            ...
            deleteRow(index, rows) {
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    delhomefm(deleteitem).then(response => {
                        //this.list = response.data.content;
                        if(response.data.code==200){
            ...
            changeSort (rows) {
                ...
                //row.splice(index, 1);
                sorthomefm(sortitem).then(response => {
                    //this.list = response.data.content;
                    if(response.data.code==200){
                        this.disable = true;
                        this.$message({
